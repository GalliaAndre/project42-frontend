      ...
      <v-card>
        ...
        <v-card-text class="dialog-overflow">
          ...
          <v-select
            ...
            item-value="id"
            label="Series"
            no-data-text="No series created"
            clearable
          ></v-select>
...
import { Video, LibraryForm, Series } from '@/store/model';
import Backend from '@/backend';
import { objectToFormData } from 'object-to-formdata';

@Component
export default class LibraryVideoEdit extends Vue {
  // Input
  @Prop() readonly categoryDescriptions: string[];
  @Prop() readonly librarySeries: Series[];
  @Prop() readonly buttonLabel: string;
  @Prop({ default: false }) readonly bigButton: boolean;
  @Prop() readonly defaultSeries?: number;
  ...
  onToggleDialog(showDialog: boolean): void {
    ...
    if (this.model) {
      ...
      if (this.model.series) {
        this.form.series = Number(
          this.model.series.replace('/api/series/', '')
        );
      }
    } else {
      this.form = new LibraryForm();
      this.form.series = this.defaultSeries;
  ...
  submit(): void {
    ...
    const url = this.model ? `videos/${this.model.id}` : 'videos';
